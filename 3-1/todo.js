...
const show = function (filterArg) {
    const showTodoAll = function (_array) {
        ...
        _array.forEach(element => todoMap.set(element.status, 0));
        _array.forEach(element => todoMap.set(element.status, todoMap.get(element.status) + 1));
    ...
    const showFilteredTodo = function (_array) {
        const todoMap = new Map();
        let totalCount = 0;
        _array.forEach(element => todoMap.set(element.status, 0));
        _array.forEach(element => todoMap.set(element.status, todoMap.get(element.status) + 1));
        ...
        _array.forEach(element => console.log(element.name));
    }

    if (filterArg === "all") {
        showTodoAll(todos);
    } else {
        showFilteredTodo(todos.filter(v => v.status === filterArg));
    }
}
show('all');
console.log("\n");
